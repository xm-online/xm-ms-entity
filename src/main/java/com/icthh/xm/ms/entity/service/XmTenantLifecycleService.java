...
public class XmTenantLifecycleService {
    ...
    public void changeState(XmEntity xmEntity, String nextStateKey, Map<String, Object> context) {
        if (!applicationProperties.getTenantWithCreationAccessList().contains(tenantContextHolder.getContext().getTenant()
            .orElseThrow(() -> new IllegalArgumentException("Tenant not supplied")).getTenantKey().getValue())) {
