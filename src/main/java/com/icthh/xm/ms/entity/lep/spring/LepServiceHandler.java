package com.icthh.xm.ms.entity.lep.spring;

import com.icthh.lep.api.LepInvocationCauseException;
import com.icthh.lep.api.LepKey;
import com.icthh.lep.api.LepKeyResolver;
import com.icthh.lep.api.LepManager;
import com.icthh.lep.api.LepMethod;
import com.icthh.lep.commons.DefaultLepMethod;
import com.icthh.lep.commons.DefaultMethodSignature;
...
import static com.icthh.xm.ms.entity.lep.XmLepConstants.EXTENSION_KEY_SEPARATOR;
import static com.icthh.xm.ms.entity.lep.XmLepConstants.UNUSED_RESOURCE_VERSION;
...
public class LepServiceHandler implements ApplicationContextAware {
    ...
    public Object onMethodInvoke(Class<?> targetType, Object target, Method method, Object[] args) throws Throwable {
        ...
        try {
            return getLepManager().processLep(baseLepKey, UNUSED_RESOURCE_VERSION, keyResolver, lepMethod);
    ...
    private static LepKey getBaseLepKey(LepService typeLepService, LogicExtensionPoint methodLep, Method method) {
        ...
        if (keyName != null && keyName.contains(EXTENSION_KEY_SEPARATOR)) {
            throw new IllegalArgumentException("Key name '" + keyName + "' can't contains segments separator: '"
                                                   + EXTENSION_KEY_SEPARATOR + "'");
        }

        String segments = groupName + EXTENSION_KEY_SEPARATOR + keyName;

        // create Lep key instance
        return new SeparatorSegmentedLepKey(segments, EXTENSION_KEY_SEPARATOR, XmLepConstants.EXTENSION_KEY_GROUP_MODE);
    }
