...
class CommentGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Comment entity")
        ...
        .repeat(2) {
            ...
            .get("/entity/api/comments")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new comment")
            .post("/entity/api/comments")
            .headers(headers_http_authenticated)
            .body(StringBody("""{"id":null, "userKey":"SAMPLE_TEXT", "message":"SAMPLE_TEXT", "entryDate":"2020-01-01T00:00:00.000Z"}""")).asJSON
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_comment_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created comment")
                .get("/entity${new_comment_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created comment")
            .delete("/entity${new_comment_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
